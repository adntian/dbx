...
pub async fn test_connection(state: State<'_, Arc<AppState>>, config: ConnectionConfig) -> Result<String, String> {
    ...
    let result = match probe_result {
        ...
        Ok(()) => match config.db_type {
            ...
            DatabaseType::Mysql => match db::mysql::connect(&url, connect_timeout).await {
                Ok(pool) => {
                    let _ = pool.disconnect().await;
                    Ok("Connection successful".to_string())
                }
            ...
            DatabaseType::Doris | DatabaseType::StarRocks => match db::mysql::connect_bare(&url, connect_timeout).await
            {
                Ok(pool) => {
                    let _ = pool.disconnect().await;
                    Ok("Connection successful".to_string())
            ...
            DatabaseType::Redis => {
                db::redis_driver::connect(&url, connect_timeout).await.map(|_| "Connection successful".to_string())
            }
            DatabaseType::DuckDb => {
...
pub async fn connect_db(state: State<'_, Arc<AppState>>, config: ConnectionConfig) -> Result<String, String> {
    ...
    let pool = match db_config.db_type {
        ...
        DatabaseType::Mysql => PoolKind::Mysql(db::mysql::connect(&url, connect_timeout).await?, MysqlMode::Normal),
